						...
						if TCH.schedule():	
							...
							if self.DATA.debug:
								print "CH %i: "%TCH.channel+\
									("T=%6.2fmK, "%(TCH.get_last_Temp()*1000) if TCH.get_last_Temp()<.9 else "T=%6.2f K, "%TCH.get_last_Temp()) +\
									("R=%6.2fkOhm"%(TCH.get_R()/1000) if TCH.get_R()>1500 else "R=%6.2f Ohm"%TCH.get_R() )
